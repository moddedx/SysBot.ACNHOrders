...
namespace SysBot.ACNHOrders
{
    ...
    public class VillagerModule : ModuleBase<SocketCommandContext>
    {
        ...
        private async Task ReplyVillagerName(GameStrings strings, string villagerName)
        {
            ...
            if (!Globals.Bot.Config.DodoModeConfig.LimitedDodoRestoreOnlyMode)
            {
                await ReplyAsync($"To order `{villagerName}` please use the following command: `{Globals.Bot.Config.Prefix}order villager:{result.Key}`. If you are ordering items and a villager add `villager:{result.Key}` to the end of the order.").ConfigureAwait(false);
            }
            else
            {
