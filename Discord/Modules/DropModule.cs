...
namespace SysBot.ACNHOrders
{
    ...
    public class DropModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task RequestRestoreLoopDodoAsync()
        {
            ...
            {
                await ReplyAsync($"{Context.User.Mention} - You have been banned for abuse. We can not send you the dodo code.");
                return;
            }
            try
            {
                ...
                await ReplyAsync($"`{Context.User.Username}`: Sent you the dodo code via DM");
                if (Globals.Bot.Config.DodoModeConfig.SentDodoChannels > 0)
                {
